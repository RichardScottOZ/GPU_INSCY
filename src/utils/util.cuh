...
using namespace std;  // kept to match existing unqualified types in this project
...
#ifdef __CUDACC__
// CUDA kernels (visible only to NVCC)
__global__ void print_array_gpu(int *x, int n);
__global__ void print_array_gpu(float *x, int n);
__global__ void print_array_gpu(bool *x, int n);
...
// Comparator used in std::map keys
struct vec_cmp {
